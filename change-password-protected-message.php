...
function pipdig_cppm_filter_text($output) {
	...
	// look for <form and prepend our custom message to it
	$output = str_replace('<form', '<div id="cppm_message_'.get_the_ID().'" class="cppm_message post-password-message" style="margin-bottom: 10px;">'.wp_kses_post(wpautop($message)).'</div><form', $output);
	
	//$output = '<div id="cppm_message_'.get_the_ID().'" class="cppm_message post-password-message">'.wp_kses_post(wpautop($message)).'</div>'.$output;
